    ...
    public synchronized void add(Class<?> cls, EventListener eventListener)
    {
        if (!cls.isInstance(eventListener))
        {
            throw new IllegalArgumentException(
                new StringBuffer().append("Listener ").append(eventListener).append(" is not of type ").append(
                    cls).toString());
        }
        if (eventListener == null)
        {
            throw new IllegalArgumentException(
                new StringBuffer().append("Listener ").append(eventListener).append(" is null").toString());
        }
