/**
 * Copyright (C) 2015 Shuang Chen This program is free software: you can redistribute it and/or modify it under the
...
/**
 * Rubik's Cube Solver.<br> A much faster and smaller implemention of Two-Phase Algorithm.<br> Symmetry is used to
 * reduce memory used.<br> Total Memory used is about 1MB.<br>
 *
 * @author Shuang Chen
 */
public class Search {
    ...
    private CubieCube[] preMoveCubes = new CubieCube[MAX_PRE_MOVES + 1];

    private int[] preMoves = new int[MAX_PRE_MOVES];

    private int preMoveLen = 0;

    private int maxPreMoves = 0;

    private boolean isRec = false;

    /**
     * Verbose_Mask determines if a " . " separates the phase1 and phase2 parts of the solver string like in F' R B R L2
     * F . U2 U D for example.<br>
     */
     ...
     *
     * @param facelets
     *            is the cube definition string format.<br> The names of the facelet positions of the cube:
     *
     *            <pre>
     *             |************|
     *             |*U1**U2**U3*|
     ...
     *            </pre>
     *
     *            A cube definition string "UBL..." means for example: In position U1 we have the U-color, in position
     *            U2 we have the B-color, in position U3 we have the L color etc. For example, the "super flip" state is
     *            represented as <br>
     *
     *            <pre>
     ...
     *            </pre>
     *
     *            and the state generated by "F U' F2 D' B U R' F' L D' R' U' L U B' D2 R' F U2 D2" can be represented
     *            as <br>
     *
     *            <pre>
     ...
     *            </pre>
     *
     *            You can also use {@link ch.min2phase.Tools#fromScramble(java.lang.String s)} to convert the scramble
     *            string to the cube definition string.
     * @param maxDepth
     *            defines the maximal allowed maneuver length. For random cubes, a maxDepth of 21 usually will return a
     *            solution in less than 0.02 seconds on average. With a maxDepth of 20 it takes about 0.1 seconds on
     *            average to find a solution, but it may take much longer for specific cubes.
     * @param probeMax
     *            defines the maximum number of the probes of phase 2. If it does not return with a solution, it returns
     *            with an error code.
     * @param probeMin
     *            defines the minimum number of the probes of phase 2. So, if a solution is found within given probes,
     *            the computing will continue to find shorter solution(s). Btw, if probeMin > probeMax, probeMin will be
     *            set to probeMax.
     * @param verbose
    ...
    private int phase1PreMoves(int maxl, int lm, CubieCube cc, int ssym) {
        ...
        if (isRec ? depth1 == length1 - preMoveLen : (preMoveLen == 0 || (0x36FB7 >> lm & 1) == 0)) {
            depth1 = length1 - preMoveLen;
            phase1Cubie[0] = cc;
            allowShorter = depth1 == MIN_P1LENGTH_PRE && preMoveLen != 0;

            if (nodeUD[depth1 + 1].setWithPrun(cc, depth1) && phase1(nodeUD[depth1 + 1], ssym, depth1, -1) == 0) {
                return 0;
            }
        }
    ...
    private int initPhase2(int p2corn, int p2csym, int p2edge, int p2esym, int p2mid, int edgei, int corni) {
        int prun = Math.max(CoordCube.getPruning(CoordCube.EPermCCombPPrun, (edgei >> 4) * CoordCube.N_COMB
                                                                            + CoordCube.CCombPConj[CubieCube.Perm2CombP[corni >> 4]
                                                                                                   & 0xff][CubieCube.SymMultInv[edgei & 0xf][corni & 0xf]]),
                Math.max(CoordCube.getPruning(CoordCube.EPermCCombPPrun, p2edge * CoordCube.N_COMB + CoordCube.CCombPConj[CubieCube.Perm2CombP[p2corn]
    ...
    private String searchopt() {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (i < 3) {
                maxprun1 = Math.max(maxprun1, urfCoordCube[i].getPrun());
            } else {
                maxprun2 = Math.max(maxprun2, urfCoordCube[i].getPrun());
            }
        }
        ...
        for (length1 = isRec ? length1 : 0; length1 < solLen; length1++) {
            ...
            if (ud.getPrun() <= length1 && rl.getPrun() <= length1 && fb.getPrun() <= length1 && phase1opt(ud, rl, fb, selfSym, length1, -1) == 0) {
                return solution == null ? "Error 8" : solution.toString();
            }
        }
        return solution == null ? "Error 7" : solution.toString();
    ...
    private int phase1opt(CoordCube ud, CoordCube rl, CoordCube fb, long ssym, int maxl, int lm) {
        ...
        for (int axis = 0; axis < 18; axis += 3) {
            ...
            for (int power = 0; power < 3; power++) {
                ...
                // UD Axis
                int prun_ud = Math.max(nodeUD[maxl].doMovePrun(ud, m, false), USE_CONJ_PRUN ? nodeUD[maxl].doMovePrunConj(ud, m) : 0);
                if (prun_ud > maxl) {
                    break;
                ...
                int prun_rl = Math.max(nodeRL[maxl].doMovePrun(rl, m, false), USE_CONJ_PRUN ? nodeRL[maxl].doMovePrunConj(rl, m) : 0);
                if (prun_rl > maxl) {
                    break;
                ...
                int prun_fb = Math.max(nodeFB[maxl].doMovePrun(fb, m, false), USE_CONJ_PRUN ? nodeFB[maxl].doMovePrunConj(fb, m) : 0);
    ...
    private int phase2(int edge, int esym, int corn, int csym, int mid, int maxl, int depth, int lm) {
        ...
        for (int m = 0; m < 10; m++) {
            ...
            int prun = CoordCube.getPruning(CoordCube.EPermCCombPPrun, (edgei >> 4) * CoordCube.N_COMB + CoordCube.CCombPConj[CubieCube.Perm2CombP[corni >> 4]
                                                                                                                              & 0xff][CubieCube.SymMultInv[edgei
                                                                                                                                                           & 0xf][corni
                                                                                                                                                                  & 0xf]]);
            if (prun > maxl + 1) {
                return maxl - prun + 1;
            ...
            prun = Math.max(CoordCube.getPruning(CoordCube.MCPermPrun, cornx * CoordCube.N_MPERM + CoordCube.MPermConj[midx][csymx]), CoordCube.getPruning(
                    CoordCube.EPermCCombPPrun, edgex * CoordCube.N_COMB + CoordCube.CCombPConj[CubieCube.Perm2CombP[cornx]
